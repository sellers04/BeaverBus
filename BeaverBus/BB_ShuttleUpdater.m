...
-(void)animateHandler
{
    ...
    for (BB_Shuttle *shuttle in shuttles) {
        ...
        if (shuttle.marker.opacity == 0){
            shuttle.marker.opacity = 0.85;
        }

        //Heading goes to 0 when at a stop, and looks strange. Try to avoid?
        if (shuttle.groundSpeed != 0 && shuttle.heading != 0){
            [shuttle.marker setRotation:([shuttle.heading doubleValue])];

            //If it was stopped, set its icon back to normal
            if ([shuttle.marker.icon isEqual:[UIImage imageNamed:[NSString stringWithFormat:@"%@_stop", shuttle.imageName]]]){
                [shuttle.marker setIcon:[UIImage imageNamed:shuttle.imageName]];
            }
        } else {
